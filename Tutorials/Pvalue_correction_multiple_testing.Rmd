...
- also called **q-value**
- To reduce potential type I errors associated with multiple testing while minimising type II errors, we controlled for false discovery rates (FDR) using a Benjamini–Hochberg procedure with a threshold of 0.2 (Verhoeven et al. 2005) (sentence from Penone 2019)
...
appropriate to set the critical α-level higher than the conven-
...
There is also a function attached which does this.
        ...
        But of corse, we need to set the level before correcting the p-values 😊 And if the new FDR level is too high, we could happen to get more significant effects than before correcting, so it should really not be too high.
